...
class AttendanceController extends Controller
{
        ...
        if ($student) {
            ...
            if ($check) {
                return [
                    'status' => 400,
                    'msg' =>
                        'Student with Matric No: ' .
                        $request->tag .
                        ' has already been signed In',
                ];
            } else {
    ...
    public function find(Request $request)
    {
        ...
        $cal = Calendar::where('id', $calendar)
            ->where('course_id', $course)
            ->with('Course')
            ->first();
        $att = Attendance::where('calendar_id', $cal->id)
            ->with('Student')
            ->get();

        return view('view-attendance', compact('att', 'cal'));
    }
    ...
    public function getById($id = null)
    {
        ...
        if ($loop) {
            foreach ($loop as $key => $value) {
                $att = Attendance::where('course_id', $value)
                    ->where('student_id', $id)
                    ->with('Courses')
                    ->get();
                // return $att;
                ...
                if ($att) {
                    ...
                    if (count($getCalendar) > 1) {
                        $rs = [
                            ...
                            'att' => count($att),
                            'attendance' => floor(
                                (count($att) / count($getCalendar)) * 100
                            ),
                        ];
                    } else {
                        $rs = [
